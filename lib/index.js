...
function weather(cityParams) {
  const city = String(cityParams);
  return new Promise((resolve, reject) => {
    // first call to get the city id
    axios.get(`http://dataservice.accuweather.com/locations/v1/cities/search?apikey=${weatherApiKey}&q=${city}`).then(response => {
      // second call to get the weather prediction
      axios.get(`http://dataservice.accuweather.com/forecasts/v1/daily/1day/${response.data[0].Key}?apikey=${weatherApiKey}`).then(prediction => {
        resolve({
          fulfillmentText: `In ${city}, ${prediction.data.Headline.Text}`,
          fulfillmentMessages: [{
            text: {
              text: [`In ${city}, ${prediction.data.Headline.Text}`]
            }
          }]
        });
      }).catch(() => {
